...
1. Read `references/rules/env.md`, `references/directory.md`, and
   `references/rules/human-docs.md`. Read `references/rules/git/` before any
   commit, branch, pull request, or issue.
...
8. Write `AGENTS.md`, including the human-document confirmation guard, and a
   `CLAUDE.md` whose entire body is `See @AGENTS.md`.
9. Show the proposed contents for `docs/human/feature-list.md`,
   `docs/human/roadmap.md`, and the current ISO-week changelog. Create or edit
   them from `references/template/` only after the human confirms that exact
   batch.
10. Set up branches per `references/rules/git/git-branch.md`.
11. Report what moved, what is now ignored, what human docs were confirmed, and
    what CI cannot run.
...
- Never create, edit, append, format, rename, move, or delete anything under
  `docs/human/` without explicit human confirmation for that exact batch.
- Do not add a `scripts/` directory at the repository root or a contributor
  guide unless the user asks.
