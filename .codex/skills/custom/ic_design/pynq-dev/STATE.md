...
Last updated: 2026-08-21T16:30:00+08:00
...
| 7. Publish Roadmap | in_progress | Claimed parent issue `#1`; created Phase issues `#2` through `#9`; prepared worktree `worktrees/npu-1-codex-a` on branch `npu/npu-1-codex-a`. | Skill validation, both strict OpenSpec validations, and `git diff --check` passed; GNU Make is unavailable on this Windows host, so RTL lint/simulation remains a documented validation blocker. |
