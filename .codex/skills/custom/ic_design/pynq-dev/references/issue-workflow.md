...
`agent-id` is a short, lowercase, stable identifier such as `a`; it must not
include a tool or vendor name. Record the issue number, URL, owner, claim
evidence, and agent id in `STATE.md`. Claiming an issue
