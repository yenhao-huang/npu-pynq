"""Bit-accurate integer contract shared by NPU export, runtime, and tests."""
...
def saturating_add_int32(accumulator: int, addend: int) -> int:
    """Saturating-add an integer to a valid signed INT32 accumulator."""

    accumulator = _require_range(
        "accumulator", accumulator, INT32_MIN, INT32_MAX
    )
    addend = _require_integer("addend", addend)
    return saturate_int32(accumulator + addend)
...
def mac_int8_int32(accumulator: int, lhs: int, rhs: int) -> int:
    ...
    return saturating_add_int32(accumulator, lhs * rhs)


def round_ratio_away_from_zero(numerator: int, denominator: int) -> int:
    """Round an integer ratio to nearest, with exact ties away from zero."""

    numerator = _require_integer("numerator", numerator)
    denominator = _require_integer("denominator", denominator)
    if denominator <= 0:
...
def requantize_int32_to_int8(
    ...
    rounded = round_ratio_away_from_zero(numerator, denominator)
